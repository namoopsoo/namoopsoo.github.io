...
- To make sure hugo allows the custom `<sup>` html you need in your config.yaml or config.toml something like      ```     markup:       goldmark:         renderer:           unsafe: true     ```
- and optionally you can spice up your hugo papermod<sup>[3](#references)</sup>, adding some color to a custom css, `assets/css/extended/my_custom_super.css` like  ```css sup a {     color: #666; } ```
